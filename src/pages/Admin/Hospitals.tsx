...
export default function AdminHospitals() {
  ...
  const handleToggleVerify = async (hospital: AdminHospital) => {
    setVerifyingId(hospital.id);
    try {
      const result = hospital.is_verified
        ? await api.unverifyAdminHospital(hospital.id)
        : await api.verifyAdminHospital(hospital.id);
      setHospitals((prev) =>
        prev.map((h) => (h.id === hospital.id ? { ...h, ...result.hospital } : h))
      );
      if (selectedHospital?.id === hospital.id) {
        setSelectedHospital({ ...selectedHospital, ...result.hospital });
      }
      toast({
        title: hospital.is_verified ? 'Verificação removida' : 'Hospital verificado',
      });
    } catch (err: unknown) {
  ...
  const openDrawer = (hospital: AdminHospital) => {
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          ...
          >
            ...
            <Button
              type="submit"
              className="bg-medical-teal text-white hover:bg-medical-accent min-w-[140px]"
              disabled={isCreating}
            >
            ...
            <tbody>
              {hospitals.map((hospital) => (
                <tr key={hospital.id} className="transition-colors hover:bg-muted/60">
                  ...
                  <td className="px-4 py-2 align-middle">
                    <Badge variant={hospital.is_verified ? 'default' : 'secondary'}>
                      {hospital.is_verified ? 'Verificado' : 'Pendente'}
                    </Badge>
                  </td>
                  <td className="px-4 py-2 align-middle">
                    ...
                    <Button
                      ...
                      onClick={() => handleToggleVerify(hospital)}
                    >
            ...
            <div className="p-4 space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div>{selectedHospital.is_verified ? 'Sim' : 'Não'}</div>
                </div>
                <div>
                  <div className="font-semibold">Cidade</div>
                  <div>{selectedHospital.city || '—'}</div>
                </div>
                <div>
                  <div className="font-semibold">CNPJ</div>
                  <div>{selectedHospital.cnpj || '—'}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <Calendar size={16} /> {selectedHospital.created_at}
                  </div>
                </div>
              </div>
            </div>
